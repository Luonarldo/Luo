...
from transformers import AutoImageProcessor, Mask2FormerForUniversalSegmentation, SegformerFeatureExtractor, SegformerForSemanticSegmentation
    ...
    def __init__(self, 
        ...
        if self.dataset_name == 'mapillary':   
            ...
            if self.model_type == 'mask2former':
                ...
                raise ValueError("Cityscapes 仅支持 mask2former 或 segformer 模型")
        elif self.dataset_name == 'ade20k':
            if self.model_type == 'mask2former':
                self.seg_processor = AutoImageProcessor.from_pretrained(
                    "facebook/mask2former-swin-large-ade20k-semantic"
                )
                self.seg_model = Mask2FormerForUniversalSegmentation.from_pretrained(
                    "facebook/mask2former-swin-large-ade20k-semantic"
                ).to(self.device)
                ...
                raise ValueError("ADE20K 仅支持 mask2former 或 segformer 模型")
        else:
